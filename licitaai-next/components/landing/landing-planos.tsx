    ...
    <section className="py-24 bg-[#050D1A] relative" id="planos">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          ...
          {/* Toggle Mensal/Anual */}
          <div className="inline-flex items-center gap-3 glass-card px-4 py-2 rounded-full border border-white/10">
            <button
              onClick={() => setAnual(false)}
              className={`text-sm font-medium px-3 py-1 rounded-full transition-all ${!anual ? "bg-blue-600 text-white" : "text-slate-400 hover:text-white"}`}
            >
              Mensal
            </button>
            <button
            ...
            return (
              ...
              >
                ...
                {plano.badge && (
                  <div
                    className={`absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 text-xs font-bold px-4 py-1.5 rounded-full uppercase tracking-widest border ${
                      isDestaque
                        ? "bg-gradient-to-r from-blue-500 to-violet-500 text-white shadow-lg shadow-blue-500/30 border-transparent"
                ...
                <div className="mb-8">
                  ...
                  {anual && plano.id === "pro" && (
                    <p className="text-xs text-emerald-400 mt-1 font-medium">Cobrado anualmente — economia de R$ 240/ano</p>
                  )}
                ...
                <ul className="space-y-4 flex-1 mb-8">
                  {plano.recursos.map((r) => (
                    <li key={r} className="flex items-start gap-3">
                      ...
                      <span className={`text-sm ${isDestaque ? "text-white" : "text-slate-300"}`}>{r}</span>
                    </li>
                  ))}
                ...
                <Link
                  ...
                  className={`w-full h-14 font-bold rounded-xl transition-all flex items-center justify-center gap-2 text-base ${
                    isDestaque
                      ...
                      : "bg-gradient-to-r from-blue-600 to-violet-600 text-white hover:shadow-[0_4px_20px_rgba(99,102,241,0.4)] hover:scale-[1.02]"
                  }`}
                >
                  {plano.cta}

...
// Cron de sincronização BNC → Supabase
...
async function executar(req: NextRequest): Promise<NextResponse> {
  ...
  const { licitacoes, total: buscadas, erros } = await fetchBnc()
  ...
  if (licitacoes.length > 0) {
    ...
    if (qtdExistentes === licitacoes.length) {
      ...
      console.log(`[cron/licitacoes-bnc] ${qtdExistentes}/${licitacoes.length} já existiam — banco atualizado, sem upsert necessário`)
    } else {
  ...
  console.log("[cron/licitacoes-bnc] Concluído:", resumo)

  await supabase.from("agent_logs").insert({
    agent: "cron/licitacoes-bnc",
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("[cron/licitacoes-bnc] Erro fatal:", msg)

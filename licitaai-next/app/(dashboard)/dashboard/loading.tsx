    ...
    <div className="space-y-8 animate-pulse">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="rounded-xl border border-slate-700 bg-slate-800 p-5 shadow-sm h-28">
      ...
      <div className="rounded-xl border border-slate-700 bg-slate-800 p-6 shadow-sm h-56">
        <div className="h-4 w-40 rounded bg-slate-700 mb-6" />
        <div className="flex items-end gap-4 h-32">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="flex-1 rounded-t bg-slate-700" style={{ height: `${30 + i * 12}%` }} />
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        {Array.from({ length: 2 }).map((_, i) => (
          <div key={i} className="rounded-xl border border-slate-700 bg-slate-800 p-6 shadow-sm space-y-4">
